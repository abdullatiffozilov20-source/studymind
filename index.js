...
// GameSet — AI tomonidan generatsiya qilingan turli formatdagi mustahkamlash o'yinlari/testlari
const GameSetSchema = new mongoose.Schema({
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
  subjectId: mongoose.Schema.Types.ObjectId, subjectName: String, topic: String,
  format: { type: String, enum: ['mcq', 'truefalse', 'fillblank', 'matching', 'wordscramble'], required: true },
  items: [{
    question: String,
    options: [String],
    correctAnswer: String,
    pairs: [{ left: String, right: String }], // matching uchun
    explanation: String
  }],
  lastScore: Number, attemptsCount: { type: Number, default: 0 },
  createdAt: { type: Date, default: Date.now }
})
...
const GameSet = mongoose.model('GameSet', GameSetSchema)
...
// ── GAMES / QUIZ (turli formatdagi mustahkamlash) ───────────────
async function generateGameSet(user, subjectName, topic, format, count=6) {
  const lang = user?.lang || 'uz'
  const langNote = lang==='uz'?'O\'zbek tilida.':lang==='ru'?'На русском.':'In English.'
  const ctx = topic ? `"${subjectName}" fanidan "${topic}" mavzusi bo'yicha` : `"${subjectName}" fani bo'yicha`

  let formatSpec = ''
  if (format==='mcq') formatSpec = `Har biri uchun 4 ta variant va to'g'ri javob:\n[{"question":"...","options":["A","B","C","D"],"correctAnswer":"A","explanation":"qisqa izoh"}]`
  else if (format==='truefalse') formatSpec = `[{"question":"...","correctAnswer":"true yoki false","explanation":"qisqa izoh"}]`
  else if (format==='fillblank') formatSpec = `Savol matnida bo'shliq ___ bilan ko'rsatilsin:\n[{"question":"... ___ ...","correctAnswer":"so'z","explanation":"qisqa izoh"}]`
  else if (format==='matching') formatSpec = `Juftliklarni moslashtirish (masalan atama-tarif):\n[{"question":"Mosla","pairs":[{"left":"...","right":"..."}],"explanation":""}]`
  else if (format==='wordscramble') formatSpec = `Har biri uchun bitta so'z/atama va uning ta'rifi (so'z keyinroq aralashtiriladi):\n[{"question":"ta'rif yoki ishora","correctAnswer":"SOZ","explanation":"qisqa izoh"}]`

  const system = `${ctx} ${count} ta savol tuz. ${langNote}
FAQAT JSON massiv qaytar, hech narsa qo'shma, izoh yozma.
${formatSpec}
Savollar mavzuga mos, xilma-xil va foydali bo'lsin.`
  const reply = await ai([{role:'user',content:'Generatsiya qil'}], system, 1400)
  const text = reply.replace(/```json|```/g,'').trim()
  try { return JSON.parse(text) } catch { return [] }
}

app.post('/api/games/generate', auth, async (req,res) => {
  const { subjectId, subjectName, topic, format, count } = req.body
  if (!subjectName || !format) return res.status(400).json({error:'subjectName va format kerak'})
  const items = await generateGameSet(req.u, subjectName, topic, format, count||6)
  if (!items.length) return res.status(500).json({error:'Yaratilmadi, qayta urinib ko\'ring'})
  const set = await GameSet.create({ userId:req.u._id, subjectId, subjectName, topic, format, items })
  res.json(set)
})
app.get('/api/games', auth, async (req,res) => {
  const q = { userId: req.u._id }
  if (req.query.subjectId) q.subjectId = req.query.subjectId
  res.json(await GameSet.find(q).sort({createdAt:-1}).limit(20))
})
app.get('/api/games/:id', auth, async (req,res) => {
  const set = await GameSet.findOne({_id:req.params.id,userId:req.u._id})
  if (!set) return res.status(404).json({error:'Topilmadi'})
  res.json(set)
})
app.post('/api/games/:id/submit', auth, async (req,res) => {
  const { score } = req.body
  const set = await GameSet.findOneAndUpdate({_id:req.params.id,userId:req.u._id},{lastScore:score,$inc:{attemptsCount:1}},{new:true})
  const xp = Math.round((score/100)*15)
  const res2 = await giveXP(req.u._id, xp)
  res.json({set, xpEarned:xp, newAchievements:res2.newAchievements})
})
app.delete('/api/games/:id', auth, async (req,res) => { await GameSet.findOneAndDelete({_id:req.params.id,userId:req.u._id}); res.json({ok:true}) })
